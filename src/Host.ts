...
class Host {
  ...
  removeSpecialRole(role: string) {
    ...
    this.removeRoleFromActiveSpecialRoles(role as Role);
  ...
  private removeRoleFromActiveSpecialRoles(targetRole: Role) {
    for (let team in this.activeSpecialRoles) {
      const teamArray = this._activeSpecialRoles
        .get(team)
        ?.filter((activeRole) => activeRole !== targetRole);
      if (teamArray) this._activeSpecialRoles.set(team, teamArray);
    }
  }
}
