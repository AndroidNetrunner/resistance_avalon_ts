...
class Dealer {
    ...
    private _playerAgreed: User[] = [];
    private _playerDisagreed: User[] = [];
    ...
    get allPlayersVoted() {
        return this._playerAgreed.length + this._playerDisagreed.length >= this._playerList.length
    }

    get allPlayersSubmittedMission() {
        return this._missionSuccess + this._missionFail >= this._proposedTeam.length;
    }

    set newTeamLeader (player: Player) {
        this._proposedTeam = [];
        this._playerAgreed = [];
        this._playerDisagreed = [];
    ...
    set agree (user: User) {
        this._playerAgreed.push(user);
    }

    set disagree (user: User) {
        this._playerDisagreed.push(user);
    }
    
    get isMissionSuccessful () {
        return (this._playerList.length >= 7 && this._roundNumber === 4 && this._missionFail < 2) || !this._missionFail;
    }

    private async notifyTurnToTeamLeader() {
        const embed = new MessageEmbed()
        .setTitle(`${this._teamLeader.user.username}님, 이제 원정대원을 지목해주실 차례입니다.`)
        .setDescription(`이번 라운드에서 지목하실 원정대원 수는 ${this._numberOfTeammates}명입니다.`)
        .setFields({
            name: `각 이모티콘이 의미하는 플레이어는 다음과 같습니다.`,
            value: this._playerList.map(player => `${player.emoticon}: ${player.user.username}`).join()
        });
        ...
        const filter = (reaction:MessageReaction, user: User) => this._teamLeader.user === user && this._playerList.map(player => player.emoticon).includes(reaction.emoji.toString());
        message.createReactionCollector({max: this._numberOfTeammates, filter, dispose: true})
            .on('collect', (reaction) => {this.addPlayerToTeam(this._playerList[this._playerList.map(player => player.emoticon).indexOf(reaction.emoji.toString())]); this._channelStartedGame.send(`${this._playerList[this._playerList.map(player => player.emoticon).indexOf(reaction.emoji.toString())].user.username}님이 원정대에 추가되었습니다.`)})
            .on('remove', (reaction) => { this.removePlayerFromTeam(this._playerList[this._playerList.map(player => player.emoticon).indexOf(reaction.emoji.toString())]); this._channelStartedGame.send(`${this._playerList[this._playerList.map(player => player.emoticon).indexOf(reaction.emoji.toString())].user.username}님이 원정대에서 제외되었습니다.`)})
            .on('end', async () => {message.delete(); await this.startVote(this._teamLeader)});
    }

    private async startVote(player: Player) {
        this._teamLeader = player;
        this._playerList.forEach((player: Player) => this.sendVoteToPlayers(player));
        this._emitter.on('completeVote', this.revealVotes);
    }
    ...
    private async startMission() {
        ...
        for (let player of this._proposedTeam) {
            ...
            message.createMessageComponentCollector().on('collect', (interaction) => {
                ...
                if (this.allPlayersSubmittedMission)
                    this._emitter.emit('missionEnd');
            })
            .on('end', () => {message.delete()});
        }
        this._emitter.on('missionEnd', this.revealMissionResult);
    }

    private revealVotes() {
        const fields = [{
            name: '찬성',
            value: `${this._playerAgreed.length}표: ${this._playerAgreed.map(user => user.username).join()}`
        }, {
            name: '반대',
            value: `${this._playerDisagreed.length}표: ${this._playerDisagreed.map(user => user.username).join()}`
        }];
        const embed = new MessageEmbed()
            .setTitle(`개표 결과, 이번 원정대는 ${this._playerAgreed.length > this._playerDisagreed.length ? '가' : '부'}결되었습니다.`)
            .setDescription(`원정대장: ${this._teamLeader.user.username}\n
            원정대: ${this._proposedTeam.map(player => player.user.username).join()}`);
        this.handOverNextLeader();
        if (this._playerAgreed.length > this._playerDisagreed.length) {
            fields.push({
        ...
        if (this._rejectedTeams === 5)
            this._emitter.emit(`gameEnd`, '5연속 원정대 부결로 인한 악의 하수인 승리');
        else if (this._playerAgreed.length <= this._playerDisagreed.length)
            this.notifyTurnToTeamLeader();
    }
    ...
    private revealMissionResult() {
        const description = this.isMissionSuccessful ? '원정대는 미션에 성공하였습니다!' : '아쉽게도 원정대는 미션에 실패하였습니다...';
        const embed = new MessageEmbed()
        ...
        .setFields([{
            ...
            value: this._proposedTeam.map(player => player.user.username).join()
        }])
    ...
    private async sendVoteToPlayers(player : Player) {
        const embed = new MessageEmbed()
            .setTitle(`${this._teamLeader.user.username}님이 ${this._roundNumber}라운드 ${this._rejectedTeams + 1}번째 원정대를 제안하였습니다.`)
            .setDescription(`아래 버튼을 눌러 찬성 혹은 반대를 투표해주세요.`)
            .addField(`원정대에 소속된 플레이어들은...`,
            `${this._proposedTeam.map(player => player.user.username).join()} 입니다!`);
            ...
            message.createMessageComponentCollector({max: 1})
                .on('collect', interaction => {
                    ...
                    if (this.allPlayersVoted)
                        this._emitter.emit('completeVote');
                })
                .on('end', () => {message.delete()});
    }
    private notifyVoteSaved(interaction: MessageComponentInteraction) {
        ...
        interaction.user.send(`${interaction.customId === 'agree' ? '찬성' : '반대'}에 투표하셨습니다.`);
        this._channelStartedGame.send(`${interaction.user.username}님이 투표하셨습니다.`);
    }
}
