...
const makeEmbed = (
    ...
    visibleRole?: Role[]
        ...
        const visiblePlayers = visibleRole && playerList.filter(opponent => player !== opponent && opponent.role in visibleRole).map(opponent => opponent.user.username);
...
const makeEmbedContent = (role: Role, visiblePlayers?: string) : content => {
    switch (role) {
        case roles.Merlin:
            return {
                role: roles.Merlin,
                description: '모드레드를 제외한 악의 하수인들을 알 수 있지만,\n당신이 암살당한다면 선의 세력은 패배합니다!',
        ...
        case roles.Loyal:
            return {
                role: roles.Loyal,
                description: "다른 선의 세력을 파악해 미션을 성공시켜 게임에서 승리하세요!",
                fieldName: "",
                fieldValue: ""
            }
        case roles.Evil: 
        ...
        case roles.Percival: 
            return {
                role: roles.Percival,
                description: "멀린이 누구인지 알 수 있지만, 모르가나도 위장한 채 당신에게 보일 것입니다!",
        ...
        case roles.Assassin: 
            return {
                role: roles.Assassin,
                description: "악의 하수인이 패배하기 직전, 멀린 암살에 성공한다면 악의 세력은 역전승합니다!",
                fieldName: '당신의 눈에 보이는 악의 세력은...',
                fieldValue: `${visiblePlayers}입니다!`
            }
        case roles.Mordred:
            return {
                role: roles.Mordred,
                description: "당신은 악의 하수인이지만, 멀린에게 정체가 보이지 않습니다!",
                fieldName: '당신의 눈에 보이는 악의 세력은...',
                fieldValue: `${visiblePlayers}입니다!`
            }
        case roles.Morgana: 
        ...
        case roles.Oberon: 
            return {
                role: roles.Oberon,
                description: "악의 하수인이지만, 다른 악의 하수인들과 정체를 서로 모릅니다.",
                fieldName: "",
                fieldValue: ""
            }
        default:
